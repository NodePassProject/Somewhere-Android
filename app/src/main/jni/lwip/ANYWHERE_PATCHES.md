...
It does not lose, reorder, or congest packets; the only real bottleneck
...
- **RTO collapse** (`tcp_slowtmr`, `src/core/tcp.c`). Any spurious
  timeout - a brief TUN output drain stall, a delayed app-side ACK, an
  executor scheduling hiccup - resets `cwnd = 1 * MSS` and halves
  `ssthresh`. Recovery then takes many RTTs of slow start.
...
other received segment and falls back to a fast timer for the tail.
...
segment bursts by up to one `tcp_fasttmr` tick (250 ms in the default
lwIP build; 100 ms with Anywhere's current `TCP_TMR_INTERVAL`). On the
in-memory TUN flow, ACK packets cost essentially nothing - they take
the `netif_output -> outputPackets -> ParcelFileDescriptor.write` path
back to the Android kernel with no real link in between - while the
delayed-ACK tail is a direct user-visible latency tax on short flows
...
hundred extra small ACK packets per second at 1 MB/s upload.
...
- `TF_ACK_DELAY` is still read by `tcp_fasttmr` (`src/core/tcp.c`) and
  ...
  paths keep working because they do not depend on `tcp_ack` ever
